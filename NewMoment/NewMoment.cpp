...
extern uint32_t NEWCET_NODE_ID;
extern std::map<uint32_t, CETNode*> CLOSED_ITEMSETS;
extern std::map<uint32_t, std::vector<std::vector<bool>*>*> TRX_INDEX;
...
void Append(const uint32_t _tid, std::vector<uint32_t>* _transaction, const uint32_t _minsupp, CETNode* const _node,
  ...
  if (_node->itemset->size() == 0 || (node_s = support(_node, _window_size, _trx_shift_left, _trx_shift_right, _BLOCK_SIZE)) > _minsupp) {
    //std::cout << "supp = " << node_s << std::endl;
    _node->support = node_s;
    _node->oldHash = _node->hash;
    _node->hash = _node->support;
    std::vector<uint32_t>* closure;
    std::vector<uint32_t>* missing_items = 0;
    ...
    if (!(closure = left_check(_node, _EQ_TABLE)) || (missing_items = inter(closure, _transaction))->size() != closure->size()) {
      ...
      if (_node->parent) {
        ...
        for(; it != siblings->end(); ++it){
          ...
          if (sibling->item > _node->item && (!_node->children || _node->children->end() == _node->children->find(sibling->item))) {
            int sibsup;
            if ((sibsup = support(sibling, _window_size, _trx_shift_left, _trx_shift_right, _BLOCK_SIZE)) > _minsupp) {
              ...
              uint32_t supp = support(_node->itemset, childBitset = compute_bitset(_node, sibling->item, _trx_shift_left, _trx_shift_right, _BLOCK_SIZE),
                _window_size, _trx_shift_left, _trx_shift_right, _BLOCK_SIZE);
              if (supp > _minsupp) {
                ...
                //std::cout << "Adding node to node" << std::endl;
              ...
              //
                //clean copy of bitset
              std::vector<std::vector<bool>*>::iterator it = childBitset->begin();
              for (; it != childBitset->end(); ++it)
                delete *it;
              delete childBitset;
            }
          }
      ...
      if (children) {
        ...
        for (; it != children->end(); ++it) {
          ...
          //should we add > item ?
          Append(_tid, _transaction, _minsupp, child, _window_size, _trx_shift_left, _trx_shift_right, _EQ_TABLE, _BLOCK_SIZE);
        }
      }
      ...
      if (children) {
        ...
        for (; it != children->end(); ++it) {
          CETNode* const child = it->second;
          uint32_t s;
          if ((s = support(child, _window_size, _trx_shift_left, _trx_shift_right, _BLOCK_SIZE)) == /*_node.support*/ node_s) {
  ...
  if (_node->itemset->size() == 0 || ((node_s = support(_node, _window_size, _trx_shift_left, _trx_shift_right, _BLOCK_SIZE))) > _minsupp) {
    ...
    if (children) {
      ...
      for (; it != children->end(); ++it) {
        ...
        //should we add > item ?
        Delete(_tid, _transaction, _minsupp, child, _window_size, _trx_shift_left, _trx_shift_right, _EQ_TABLE, _BLOCK_SIZE);
      }
    }
...
std::vector<uint32_t>* left_check_full(CETNode* const _node, std::map<long, std::vector<std::vector<CETNode*>*>*>* const _EQ_TABLE) {
  if (_EQ_TABLE->end() != _EQ_TABLE->find(_node->hash)) {
    ...
    for (uint32_t i = _node->itemset->size() + 1; i < potential_closures_by_size->size(); ++i) {
...
std::vector<std::vector<bool>*>* compute_bitset(CETNode* const _node, const uint32_t _BLOCK_SIZE) {
  ...
  //std::cout << "blocks : " << current->size() << std::endl;
...
std::vector<std::vector<bool>*>* cloneList(std::vector<std::vector<bool>*>* const _list) {
  ...
  std::vector<std::vector<bool>*>::iterator it = _list->begin();
  for (; it != _list->end(); ++it) {
    std::vector<bool>* const item = (*it);
    clone->push_back(new std::vector<bool>(item->begin(), item->end()));
  }
  return clone;
};
...
std::vector<std::vector<bool>*>* compute_bitset(CETNode* const _node, const uint32_t _item, const uint32_t _trx_shift_left, const uint32_t _trx_shift_right, const uint32_t _BLOCK_SIZE) {
  ...
  std::vector<std::vector<bool>*>* const item_bitset = cloneList(TRX_INDEX.find(_item)->second);//here copy is useless
  intersect_bitsets(bitset_parent_left, item_bitset, _BLOCK_SIZE);
  //clean copy of bitset
  std::vector<std::vector<bool>*>::iterator it = item_bitset->begin();
  for (; it != item_bitset->end(); ++it)
    delete *it;
  delete item_bitset;
...
uint32_t support(CETNode* const _node, const uint32_t _window_size, const uint32_t _trx_shift_left, const uint32_t _trx_shift_right, const uint32_t _BLOCK_SIZE) {
  std::vector<std::vector<bool>*>* const temp = compute_bitset(_node, _BLOCK_SIZE);
  const uint32_t supp = support(_node->itemset, temp, _window_size, _trx_shift_left, _trx_shift_right, _BLOCK_SIZE);
  //clean bitset
  std::vector<std::vector<bool>*>::iterator it = temp->begin();
  for (; it != temp->end(); ++it)
    delete *it;
  delete temp;
  return supp;
...
uint32_t support(std::vector<uint32_t>* const _itemset, std::vector<std::vector<bool>*>* const _bitset, const uint32_t _window_size, const uint32_t _trx_shift_left, 
  ...
  //std::cout << _window_size << " " << _BLOCK_SIZE << std::endl;
    ...
    if (_i < _window_size) {
      for (uint32_t j = 0; j != _transaction->size(); ++j) {
        if (TRX_INDEX.find(_transaction->at(j)) == TRX_INDEX.end()) {
          TRX_INDEX.emplace(_transaction->at(j), new std::vector<std::vector<bool>*>());
        }
        ...
        if ((TRX_INDEX.find(_transaction->at(j))->second)->empty()) {
          ...
          for (uint32_t z = 0; z != nbr_blocks; ++z) {
            std::vector<bool>* const next_block = new std::vector<bool>();
            next_block->resize(_BLOCK_SIZE);
            std::fill(next_block->begin(), next_block->end(), false);
      ...
      if (_trx_shift == 0) {
        ...
        for (uint32_t j = 0; j != _MAX_ATTRIBUTES; ++j) {
          ...
          if (binary_search(_transaction->begin(), _transaction->end(), j)) {
            if (TRX_INDEX.find(j) == TRX_INDEX.end()) {
              TRX_INDEX.emplace(j, new std::vector<std::vector<bool>*>());
            }
            //si on doit creer un nouveeau block ?
            if ((TRX_INDEX.find(j)->second)->empty()) {
              std::vector<bool>* const new_block = new std::vector<bool>();
              new_block->resize(_BLOCK_SIZE);
              std::fill(new_block->begin(), new_block->end(), false);
            ...
            if (TRX_INDEX.find(j) != TRX_INDEX.end() && !(TRX_INDEX.find(j)->second)->empty()) {
              ...
              //Arrays.fill(first_block, false);
              std::fill(first_block->begin(), first_block->end(), false);
              std::vector<std::vector<bool>*>* const lol = (TRX_INDEX.find(j)->second);
              lol->erase(lol->begin());//TODO: rewrite this
              lol->push_back(first_block);
              //System.out.println(j + " - replaced oldest block at " + (TRX_INDEX.get(j).size() - 1));
              //System.out.println("" + TRX_INDEX.get(j).toString());
            }
          }
        ...
        for (uint32_t j = 0; j != _transaction->size(); ++j) {
          ...
          if ((TRX_INDEX.find(_transaction->at(j))->second)->empty()) {
            ...
            const uint32_t nbr_blocks = ((uint32_t)std::ceil(_window_size / (float)_BLOCK_SIZE));
            for (uint32_t z = 0; z != nbr_blocks; ++z) {
              std::vector<bool>* const next_block = new std::vector<bool>();// BLOCK_SIZE];
              next_block->resize(_BLOCK_SIZE);
              std::fill(next_block->begin(), next_block->end(), false);
